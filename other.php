...
if (!empty($userId)) {
    ...
    $contactsSql = "SELECT * FROM `contacts2` WHERE `owner_id`= $userId ORDER BY first_name ASC LIMIT {$offset}, {$limit}";
    $conn = db_conn();
...
while ($row = mysqli_fetch_assoc($contactsResult)) {
            $userImage = !empty($row['photo']) ? SITEURL . "uploads/photo/" . $row['photo'] : "https://via.placeholder.com/50.png/09f/666";
            ?>
      <tr>
