...
if (isset($_POST) && !empty($_SESSION['user'])) {
    ...
    if (!empty($cId)) {
        ...
        if (!empty($photoName)) {
            $sql = "UPDATE `contacts2` SET first_name ='{$firstName}', last_name='{$lastName}', email='{$email}',phone='{$phone}', address='{$address}', photo='{$photoName}' WHERE id={$cId} AND owner_id={$ownerId}";
        } else {
            // if no photo selected
            $sql = "UPDATE `contacts2` SET first_name ='{$firstName}', last_name='{$lastName}', email='{$email}',phone='{$phone}', address='{$address}' WHERE id={$cId} AND owner_id={$ownerId}";
        }
        $message = "Contact has been updated successfully!";
    } else {
        //insert new record
        $sql = "INSERT INTO `contacts2` (first_name, last_name, email, phone, address, photo, owner_id) VALUES ('{$firstName}','{$lastName}','{$email}','{$phone}', '{$address}', '{$photoName}','{$ownerId}')";
